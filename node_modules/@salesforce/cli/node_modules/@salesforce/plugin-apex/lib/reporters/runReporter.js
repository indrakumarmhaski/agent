...
export default class RunReporter {
    static formatDefault(response) {
        const outputText = [];
        if (response.success) {
            outputText.push(colorSuccess(messages.getMessage('executeCompileSuccess')), colorSuccess(messages.getMessage('executeRuntimeSuccess')), '', response.logs ?? '');
        }
        else {
            ...
            if (!response.compiled) {
                outputText.push(colorError(`Error: Line: ${diagnostic.lineNumber}, Column: ${diagnostic.columnNumber}`), colorError(`Error: ${diagnostic.compileProblem}\n`));
            }
            else {
                outputText.push(colorSuccess(messages.getMessage('executeCompileSuccess')), colorError(`Error: ${diagnostic.exceptionMessage}`), colorError(`Error: ${diagnostic.exceptionStackTrace}`), '', response.logs ?? '');
            }
        }
