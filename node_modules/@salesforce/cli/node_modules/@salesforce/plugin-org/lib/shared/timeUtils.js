...
export const getClockForSeconds = (timeInSec) => {
...
export const getTimeComponentsFromSeconds = (timeInSec) => {
    const days = Duration.days(Math.floor(timeInSec / 86400));
    const hours = Duration.hours(Math.floor((timeInSec % 86400) / 3600));
    const minutes = Duration.minutes(Math.floor((timeInSec % 3600) / 60));
    const seconds = Duration.seconds(Math.floor(timeInSec % 60));
    return { days, hours, minutes, seconds };
};
export const getSecondsToHuman = (timeInSec) => {
    const tc = getTimeComponentsFromSeconds(timeInSec);
