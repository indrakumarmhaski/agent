...
import { API, isSourceComponent } from '../../../utils/types.js';
...
export class Source extends SfCommand {
    ...
    async delete() {
        ...
        if (sourcepaths) {
            ...
            await Promise.all(this.components
                ...
                .flatMap((bundle) => sourcepaths.map((sourcepath) => 
    ...
    async maybeUpdateTracking() {
        if (this.flags['track-source'] ?? false) {
            ...
            const successes = (this.fileResponses ?? this.deployResult.getFileResponses()).filter((fileResponse) => fileResponse.state !== ComponentStatus.Failed);
            if (!successes.length) {
                this.spinner.stop();
                return;
            }
    ...
    async deleteFilesLocally() {
        if (!this.flags['check-only'] && this.deployResult?.response?.status === RequestStatus.Succeeded) {
            ...
            this.components?.filter(isSourceComponent).map((component) => {
